...
public:
	...
	void change_info(std::string new_value, std::string param_to_change, std::string where, std::string what_change, std::string user_name, std::string user_surname) {
		...
		pqxx::result search_user(std::string user_name, std::string user_surname, std::string user_phone, std::string user_mail) {
			pqxx::work tx(connect);
			std::string query = "SELECT name, surname, mail_address, phone_number FROM users "
	...
	int main() {
		try
		{
			...
			pqxx::result res = db_worker.search_user("", "", "+79991234567", "");

			for (auto row : res) {
				std::cout << "Found: " << row["name"].c_str() << " " << row["surname"].c_str()
